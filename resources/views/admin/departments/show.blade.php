    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            ...
            {{-- <ul class="nav nav-tabs" role="tablist">
                <li role="presentation" class="active"><a href="#employees" aria-controls="employees" role="tab" data-toggle="tab">@lang('quickadmin.employees.title')</a></li>
            </ul> --}}
            ...
            {{-- <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane active" id="employees"> --}}
                    <table class="table table-bordered table-striped {{ count($employees) > 0 ? 'datatable' : '' }}">
                        <thead>
                            <tr>
                                <th>@lang('quickadmin.employees.title')</th>
                            ...
                            @if (count($employees) > 0)
                                @foreach ($employees as $employee)
                                    <tr data-entry-id="{{ $employee->id }}">
                                        <td field-key='title'>{{ $employee->title }}</td>
                                                    <td field-key='first_name'>{!! $employee->first_name !!}</td>
                                                    <td field-key='last_name'>{{ $employee->last_name or '' }}</td>
                                                    <td field-key='job_title'>{{ $employee->job_title }}</td>
                                                    <td field-key='email'>{{ $employee->email or '' }}</td>
                                                    <td field-key='phone'>{{ $employee->phone or '' }}</td>
                                                    @if( request('show_deleted') == 1 )
                                                    <td>
                                                        @can('employee_delete')
                                                                                            {!! Form::open(array(
